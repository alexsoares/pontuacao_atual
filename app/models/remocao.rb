...
attr_accessor :letivo, :ano_anterior, :ano_atual


protected
def executar_remocao
      @trab_ant = Trabalhado.find_by_professor_id(self.professor_id, :conditions => 'ano = ' + self.ano_anterior.to_s + ' and ano_letivo = ' + self.letivo.to_s)
      @trab_atual = Trabalhado.find_by_professor_id(self.professor_id, :conditions =>'ano = ' + self.ano_atual.to_s + ' and ano_letivo = ' + self.letivo.to_s)
      @atualiza_trabalhado = Trabalhado.find_by_professor_id(self.professor_id, :conditions => 'ano_letivo = ' + self.letivo.to_s)
...
def finalizar_remocao
      @trab_ant = Trabalhado.find(:all, :conditions => 'professor_id = ' + (self.professor_id).to_s + ' and ano = ' + self.ano_anterior.to_s + ' and ano_letivo = ' + self.ano_letivo.to_s)
      @trab_atual = Trabalhado.find(:all, :conditions => 'professor_id = ' + (self.professor_id).to_s + ' and ano = ' + self.ano_atual.to_s + ' and ano_letivo = ' + self.ano_letivo.to_s)
      @atualiza_trabalhado = Trabalhado.find(:all, :conditions => 'professor_id = ' + (self.professor_id).to_s + ' and ano_letivo = ' + self.ano_letivo.to_s)
      @ac_t = AcumTrab.find(:all, :conditions => 'professor_id = ' + (self.professor_id).to_s)
      @ac_ta = AcumTrabAntiga.find(:all, :conditions => 'professor_id = ' + (self.professor_id).to_s)
        ...
        for k in @ac_t
          for l in @trab_ant
            for m in @trab_atual
              ...
              if (acta.tot_acum_ant_trab != k.tot_acum_ant_trab) or (acta.tot_acum_ant_efet != k.tot_acum_ant_efet) or (acta.tot_acum_ant_rede != k.tot_acum_ant_rede)
                self.total = (k.pont_base_trab * (k.tot_acum_ant_trab))
                self.total = self.total + (k.pont_base_efet * (k.tot_acum_ant_efet))
                self.total = self.total + (k.pont_base_rede * (k.tot_acum_ant_rede))
                self.total = self.total + (k.pont_base_unid * (k.tot_acum_ant_unid))
              else
